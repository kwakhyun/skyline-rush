...
const board=(b:unknown)=>b==="qa"?"qa-v6":"main-v6";
...
export async function GET(req:Request){
 try{
  const url=new URL(req.url),b=board(url.searchParams.get("board"));
  ...
  return json({season:"시즌 2 · 세 번의 도전",rules_version:6,entries:entries.results,my_rank:mine?.rank??null});
...
export async function POST(req:Request){
 try{
  ...
  const hash=await digest(JSON.stringify([body.seed,body.ticks,body.events,body.rules_version,board(body.board)]));
  ...
  await db().batch([
   ...
   db().prepare("INSERT INTO runs(id,player_id,board,score,distance,ticks,seed,replay_hash,created_at) VALUES(?,?,?,?,?,?,?,?,?)").bind(body.run_id,player.id,board(body.board),result.score,result.distance,body.ticks,body.seed,hash,Date.now())
  ]);
