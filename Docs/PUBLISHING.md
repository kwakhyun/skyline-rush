...
## 특별 구간 업데이트 — 0.7.0-preview.1

- Windows 실행 파일: https://github.com/kwakhyun/skyline-rush/releases/tag/v0.7.0-preview.1
- 다섯 테마의 특별 구간 메시·이미지 재질, 점프/슬라이드/틈의 위험 보상, 아슬아슬 보너스, 결과 점수 분해.
- 새 점수는 시즌 3 / Rules 7로 분리한다. 이전 버전 6 클라이언트와 기존 서버 기록은 별도 시즌에서 계속 지원한다. 이전 PC 저장 파일은 그대로 남긴다.
- Unreal 검증 13개, C++/서버 리플레이 4개, 실제 3.5km 입력 재생, 다섯 테마 화면 검수, 로컬 랭킹 API 검증을 완료했다. [검증과 실제 화면](QA/special-sections.md).
- Shipping 패키지에서 5개 관문 메시와 표면 이미지가 cooked 데이터에 포함되는지 확인하고, 시작 화면·시작 조작·실제 달리기를 확인했다. 아래 이전 출시 결과는 당시의 기록이다.
...
v0.7.0-preview.2는 전용 일러스트와 타이틀 메뉴를 추가합니다. Editor 컴파일, 러너 테스트 13개, 리플레이 대조, 3개 해상도별 메뉴 검증 8개씩을 통과했습니다. Shipping 빌드 및 타이틀 에셋 포함을 확인했습니다. 패키지 자체의 화면 캡처는 Computer Use 앱 접근 승인 시간 초과로 완료하지 못했습니다.
