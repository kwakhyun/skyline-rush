...
if($RunnerResults.failed -gt 0 -or $RunnerResults.succeeded -lt 12){throw 'Runner test report incomplete or failed'}
& node (Join-Path $RunnerRoot 'Tools\verify_runner_replay.mjs')
if($LASTEXITCODE -ne 0){throw 'Offline replay parity failed'}
Write-Output 'SKYLINE RUSH: 12 Unreal tests and offline ranking parity passed.'
